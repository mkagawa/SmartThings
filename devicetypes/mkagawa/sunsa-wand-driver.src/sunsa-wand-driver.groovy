    ...
    try {
      ...
      httpGet(params) { resp ->
          ...
          if(resp.data.deviceCount > 0) {
             def filtered = resp.data.devices.findAll { 
                 it.idDevice.toString() == parsed.devId 
             }
             if(filtered.length == 0) {
               httpError(404, "no such deviceId ${parsed.devId} exists")
             }
             filtered[0]
          }
      }
    ...
    try {
      ...
      httpPutJson(params) { resp ->
          if(resp.data.device.idDevice) {
              ...
              log.info "${resp.data.device.name} => level = ${level}"
...
def refresh() {
    ...
    def devInfo = apiGet()
    log.debug("refresh dev: $devInfo")
    sendEvent(name:"level", value:devInfo.Position)
